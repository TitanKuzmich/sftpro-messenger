...
const { API_DOMAIN, NODE_ENV = "development" } = process.env
...
module.exports = config => {
  try {
    const { mode, entry, optimization, host, port, module, output, plugins, proxy_url, devtool } = config
                ...
                const matches =
                  typeof cookie === "undefined"
                    ...
                    : cookie.match(
                        ...
                        new RegExp(`(?:^|; )${api_token.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, "\\$1")}=([^;]*)`)
                      )

                proxyReq.setHeader("Authorization", `Bearer ${matches ? decodeURIComponent(matches[1]) : "emptyToken"}`)
              }
              return proxyReq
            }
          }
